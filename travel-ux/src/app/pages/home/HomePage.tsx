...
import { retrieveUser, logout, registerAsUserConsumer, deregisterUserConsumer } from '../../data/services/UserService';
...
interface HomePageState extends UserConsumerState {
  records:Record[],
...
class HomePage extends React.Component<HomePageProps, HomePageState> {
  ...
  timelinePopulate(travelRecords:TravelData[]) {
    let groups:TimelineGroup[] = unique(travelRecords.map(travel=>travel.user?.username).map(username=>{return {"id":username, "content":username}}), "id");
              ...
              <div className="travelPanel">
                    <div className="travelRecordPanel">
                        {this.state.selectedTravel.map(travel=>(
                            <div key={Math.random()*9999} onClick={e => (this.setState({"selectedTravel":[travel], "selectedEntityId": this.state.records.find(record=>record.travelRecords.findIndex(tr=>tr.id === travel.id) !== -1)?.boundary[0].id}, ()=>this.timelinePopulate([travel])))} className="travelRecord">
